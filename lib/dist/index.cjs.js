...
const scanDocument = (options) => {
    return new Promise((resolve, reject) => {
        ...
        const tempFilePath = path__namespace.join(os__namespace.tmpdir(), `scanned-document-${Date.now()}.pdf`);
        ...
        for (let i = 0; i < naps2Paths.length; i++) {
            ...
            child_process.exec(command, (error, stdout, stderr) => __awaiter(void 0, void 0, void 0, function* () {
                if (error) {
                    ...
                    try {
                        const fileBuffer = fs__namespace.readFileSync(tempFilePath);
